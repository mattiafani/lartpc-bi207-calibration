...
UV_PITCH    = 4.669/sin(pi/180*(ANGLE_DEG)) # mm # wire spacing perpendicular to wire
...
def main():
    ...
    for x0 in tqdm(arange(0, apa_frame.width, 1)):
        ...
        # for y0 in arange(800*STEP_Y, 400*STEP_Y, -1*STEP_Y):
            ...
            near_wires_names, distances, _ = \
                apa_frame.evaluate_point_distance(test_point, linewidth=0.1, zorder=1, n_wires=1)
    ...
    point_distances = sorted(point_distances, key=lambda i: sum(i[1]) / len(i[1]))
    ...
    for n, point in enumerate(best_points):
        ...
        apa_frame.plot(near_wires_names,
                        save_fig_path=f'{plot_dir}Position_{str(n + 1).zfill(int(math.log10(n_best_points)) + 1)}_x'
                                      f'{round(point_distances[n][0].x,1)}_y{round(point_distances[n][0].y,1)}.pdf',
                        colors=get_colors_legend_list(wire_planes.keys(), n_wires),
                        title=f"Position {n+1}: ({point.x:.1f}, {point.y:.1f}) - "
                              f"Average distance = {round(sum(distances) / len(distances), 2)} mm",
                        distances=distances
                        )
