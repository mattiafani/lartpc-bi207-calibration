...
class WirePlane:
    ...
    def __generate_wire(self,
                        ...
                        i: int,
                        assign_femb_number: bool = False):
        wire = Wire(self.apa_frame, x, self.start_y,
                    self.deg, self.color, self.name, i)
    ...
    def __generate_plane(self,
        ...
        if self.end_x > self.start_x:
            while x <= self.end_x:
                # print(f"1 - {x}, {i}")
                self.__generate_wire(
                    x, i, assign_femb_number=assign_femb_number)
                i += 1
                x += self.pitch
