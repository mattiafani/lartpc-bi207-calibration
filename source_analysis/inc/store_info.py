...
def txt_file_initialize(path_to_folder_plots, raw_data_folder_name):
    ...
    peak_width, blip_heights_per_plane, blip_distance = get_blips_parameters(
        raw_data_folder_name[:4])
    peak_width, peak_height, n_strips_w_peaks = get_cosmic_rejection_parameters(
        raw_data_folder_name[:4])

    txt_filename = f"{path_to_folder_plots}/{raw_data_folder_name}_CR_{peak_width}_{peak_height}_{
        n_strips_w_peaks}_BH_{blip_heights_per_plane[0]}_{blip_heights_per_plane[1]}_{blip_heights_per_plane[2]}"
...
def store_evts(filename, evt_nr, raw_file_nr, raw_evt_nr, json_file_nr, json_evt_nr, strip_nr, evt_st_t, evt_pk_t, evt_en_t, charge, charge_cluster):
    ...
    with open(filename, "a") as ft:
        ft.write(f"{evt_nr},{raw_file_nr},{raw_evt_nr},{json_file_nr},{json_evt_nr},{
                 strip_nr},{evt_st_t},{evt_pk_t},{evt_en_t},{charge},{charge_cluster}\n")
