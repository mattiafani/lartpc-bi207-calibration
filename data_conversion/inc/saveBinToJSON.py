        ...
        for i_raw_file_name in filenames:
            ...
            if raw_data_folder_name.startswith(inc.settings.IGNORED_FOLDER_PREFIX):
                ...
                if i_raw_file_name.endswith(".bin") and "ADC_OFT" not in i_raw_file_name:
                    ...
                    raw_file = os.path.join(dirpath, i_raw_file_name)
                    print('\n [' + find_time_now() + '] : ' +
                          str(i_raw_file_counter) + ',', i_raw_file_name)
                    ...
                    while n_converted_events < inc.settings.N_EXPECTED_EVTS_PER_RAW_FILE and not flag_eof:
                        ...
                        for k in range(inc.settings.N_EVTS_PER_CONV_FILE):

                            i_event += 1  # from 1 to n_events
                            i_binary_event += 1  # from 1 to end_of_file (25)
                            # from 1 to inc.settings.N_EVTS_PER_CONV_FILE (10)
                            i_converted_event += 1
                            ...
                            try:
                                ...
                                print(
                                    f'{inc.settings.output_align}: i_event={
                                        i_event}, i_binary_event={i_binary_event}, '
                                    f'i_converted_event={i_converted_event}')
                                fembdata = raw_convertor_conv(
                                    raw_file, trigno=i_binary_event)
                                ...
                                for j in range(8):
                                    for l in range(16):
                                        ...
                                        y = np.array(fembdata[chn]) % 0x10000
                                        y_std[inc.settings.chanPhy[chn] -
                                              1] = np.std(y)
                                        data['chn' +
                                             str(inc.settings.chanPhy[chn] - 1)] = y.tolist()

                                        myArray = np.array(y.tolist())
                                        findPeaksCount(
                                            myArray, chn, peak_height, peak_width, sourcePosFinder, sourcePosFinderAll)
                                        ...
                                        if inc.settings.chanPhy[chn] > source_strip_c - 2 and inc.settings.chanPhy[chn] < source_strip_c + 2:
                                            source_blips, properties = findBlips(
                                                myArray, chn, blip_height, peak_width, distance=blip_distance)
                                        elif inc.settings.chanPhy[chn] < 48:
                                            blips, properties = findBlips(
                                                myArray, chn, peak_height, peak_width, distance=blip_distance)
                                        ...
                                        if inc.settings.chanPhy[chn] < 48:
                                            ...
                                            peaks, properties = findPeaks(
                                                myArray, chn, peak_height, peak_width)
                                        ...
                                        if ifSingleEvtDisplay:
                                            singleEvtDisplay(
                                                chn, withOutBaseline, peaks, source_blips, blips)
                                ...
                                print(
                                    f"{inc.settings.output_align}- EOF : i_event={
                                        i_event}, i_binary_event={i_binary_event}, "
                                    f"i_converted_event={i_converted_event}")
