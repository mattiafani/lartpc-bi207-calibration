...
def raw_convertor_conv(fp, femb_num=1, trigno=0, jumbo_flag=True):
    ...
    with open(fp, 'rb') as f:
        ...
        while (trig_i < trigno):
            link_i = 0
            links_data = [np.array([], dtype=int), np.array(
            ...
            while (addr <= len(dataNtuple) - 25) and (link_i < femb_num*4):
                ...
                # << bitwise left shift, & bitwise and
                pkg_cnt0 = (
                    (((dataNtuple[addr]) << 16) + (dataNtuple[addr+1]))) & 0xFFFFFFFF
                pkg_res0 = (
                    (((dataNtuple[addr+2]) << 16) + (dataNtuple[addr+3]))) & 0xFFFFFFFF
                a = addr
                for b in range(addr, len(dataNtuple)-4, 1):
                    pkg_cnt1 = (
                        (((dataNtuple[b]) << 16) + (dataNtuple[b+1]))) & 0xFFFFFFFF
                    pkg_res1 = (
                        (((dataNtuple[b+2]) << 16) + (dataNtuple[b+3]))) & 0xFFFFFFFF
                    if (pkg_cnt1 == pkg_cnt0 + 1) and (pkg_res0 == 0) and (pkg_res1 == 0):
                        addr = b
                        break
                udp_pkg = np.array(dataNtuple[a:b], dtype=int)

                # feed-65261, face-64206
                if (udp_pkg[8] == 0) and ((udp_pkg[9] == 0xface) or (udp_pkg[9] == 0xfeed)):
                    links_data[link_i] = np.append(
                        links_data[link_i], udp_pkg[9:])
                else:
                    links_data[link_i] = np.append(
                        links_data[link_i], udp_pkg[8:])
                if (b-a) < pkg_len-10:
                    link_i += 1
                    except_cnt = 0
                else:
                    ...
                    if except_cnt >= 10:
                        print(f"{inc.settings.output_align}! i_binary_event={
                              trig_i}: This is the last trigger event in the binary file")
        ...
        for i in range(len(links_data)):
            ...
            links_face_pos[i] = np.sort(
                np.append(links_face_pos[i], links_feed_pos[i]))
        ...
        for i in range(len(links_face_pos)):
            ...
            for j in range(len(links_face_pos[i])-1):
                if (links_face_pos[i][j+1] - links_face_pos[i][j] == 25):
                    onepkgdata = links_data[i][int(
                        links_face_pos[i][j]): int(links_face_pos[i][j]) + 25]
        ...
        femb_data = chipx2_data[0] + chipx2_data[1] + \
            chipx2_data[2] + chipx2_data[3]
